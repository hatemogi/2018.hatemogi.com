...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { au: 'hidden', z: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { au: 'mozHidden', z: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { au: 'msHidden', z: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { au: 'webkitHidden', z: 'webkitvisibilitychange' }
		: { au: 'hidden', z: 'visibilitychange' };
}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {V: fragment, W: host, aB: path, aa: port_, ad: protocol, ae: query};
	});
...
var $author$project$Main$Model = F4(
	function (key, url, route, projectFilter) {
		return {X: key, F: projectFilter, B: route, aM: url};
	});
	...
	var paths = A2(
		...
		A2(
			...
			A2(
				...
				A2($elm$core$String$split, '/', url.aB))));
	_v0$5:
	while (true) {
		if (!paths.b) {
			return 0;
...
var $author$project$Article$Article = F3(
	function (title, url, summary) {
		return {aH: summary, aK: title, aM: url};
	});
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$href(a.aM)
								]),
							_List_fromArray(
								[
									A2(
							...
							$author$project$Main$markdown(a.aH)
						]))
				]));
	};
	return A2(
...
var $author$project$Intro$Section = F3(
	function (title, url, description) {
		return {aq: description, aK: title, aM: url};
	});
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(section.aK)
								])),
							$author$project$Main$markdown(section.aq)
						]))
				]));
	};
	return A2(
...
var $author$project$Projects$Project = F7(
	function (category, year, title, url, role, tags, description) {
		return {I: category, aq: description, aD: role, aJ: tags, aK: title, aM: url, al: year};
	});
										...
										[
											...
											_List_fromArray(
												[
													...
													var _v2 = p.aM;
													if (_v2.$ === 1) {
														return $elm$html$Html$text(p.aK);
													} else {
														var url = _v2.a;
														return A2(
															$elm$html$Html$a,
											...
											$author$project$Main$markdown(p.aq)
										])),
									A2(
									$elm$html$Html$div,
									_List_fromArray(
...
var $author$project$Main$view = function (model) {
	return {
		...
		aK: 'hatemogi.com - ' + $author$project$Main$routeToTitle(model.B)
	};
};
var $author$project$Main$main = $elm$browser$Browser$application(
	{aw: $author$project$Main$init, ay: $author$project$Main$UrlChanged, az: $author$project$Main$LinkClicked, aG: $author$project$Main$subscriptions, aL: $author$project$Main$update, aN: $author$project$Main$view});
